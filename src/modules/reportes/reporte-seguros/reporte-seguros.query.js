/**
 * Query SQL del reporte mensual "Utilizas Seguros" (Reporte seguros - Giovani)
 * (fuente: .docs/MENSUALES/Reporte saldo y seguros - GIovani/Reporte seguros/
 *          Ultimo - Utilizas Seguros.sql)
 *
 * Nota: el alias de una de las subconsultas venia guardado con un byte
 * invalido (una "\u00f1" en Windows-1252 dentro de un archivo UTF-8); se
 * renombro a "ceq" para evitar problemas de codificacion en este archivo.
 *
 * Nota del analista (notas.txt): las columnas sectorista, grupo, corredor y
 * territorio que llegan en NULL deben mostrarse como "SIN ASIGNAR" (se aplica
 * en reporte-seguros.service.js al armar el Excel).
 */

module.exports = {
  /**
   * Penetracion de seguros por producto (Cartera vs Seguro) y nivel
   * comercial (sectorista/grupo/corredor/territorio) al cierre de mes.
   * @param {string} fecha - Fecha de cierre en formato YYYYMMDD (ej. '20260630')
   */
  obtenerUtilizasSeguros: (fecha) => `
    use mod_rep;
    go
    declare @tip_cod int=7, @cod_rel varchar(50)='231'      
    declare @dFechaMax date='${fecha}'   
    ...
    select a.SCODDESCRIP,a.DESCRIPCION,a.RDESGRU,a.RDESCOR,a.RDESTER,
    	   ...
    	   --------
    	   isnull(CE,0) as 'T_Ope_CE',ceq.SNUMOPE as 'T_SEG_CE_T',case when isnull(CE,0) =0 then 0 else CAST(ceq.SNUMOPE AS FLOAT) /isnull(CE,0) end AS 'PEN_CE',
    	   ---------
    ...
    left join (select SCODDESCRIP,/*seg,*/sum(SNUMOPE)SNUMOPE from #Data where tip='Seguro'  and RCODPROD in(4,2,5) and seg in(200,802,803) group by SCODDESCRIP/*,seg*/)c
     ...
     ON A.SCODDESCRIP=n.SCODDESCRIP
          left join (select SCODDESCRIP,/*seg,*/sum(SNUMOPE)SNUMOPE from #Data where tip='Seguro'  and RCODPROD in(3) and seg in(802) group by SCODDESCRIP )ceq
     ON A.SCODDESCRIP=ceq.SCODDESCRIP
